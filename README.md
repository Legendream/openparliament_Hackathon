...
兩種格式最後都收斂成同一組 `occupation_canon`（13 類），網站的職業圖現階段不必改
（切換門檻與後續做法見下一段）。
...
目前是**累積圖**（全部場次混在一起），因此新舊定義是混合的，而且混合比例每個月都在變。
處理方式分兩階段：

| 階段 | 新格式筆數 | 做法 |
|---|---|---|
| 現在 | < 100 | 維持單一累積圖，圖上標明「多少份用新定義」（數字由 `summary.json` 的 `occupation_new_format_n` / `_pct` 算出，不寫死） |
| 切換後 | ≥ 100 | 拆成兩張並列：新格式一張、舊格式一張，各自標明涵蓋期間與定義。**舊圖不下架** |

- **100 筆的理由**：13 個分類要能講佔比，平均每類至少 7–8 筆（100÷13≈7.7）才不會整張圖
  都是個位數。至於還要幾場，各場回收量落差很大（近三場為 47、17、10 筆），
  90 筆的缺口約需 2～9 場（90÷47≈1.9、90÷10=9.0）；以中位數 17 筆計需 6 場
  （90÷17≈5.3，5 場只有 85 筆還不到門檻）。
- **舊圖為什麼不下架**：資助方可能回頭引用歷史累積數字，下架會讓他們手上的舊資料對不上。
- **不必記在腦子裡**：`build_analysis.py` 每次都會印出離門檻還差幾筆，達標時會主動提醒切換
  （門檻常數 `OCC_SWITCH_THRESHOLD`）。
